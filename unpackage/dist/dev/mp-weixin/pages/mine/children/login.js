...
var _default = { data: function data() {return { //logo图片 base64
      ...
      try {var value = uni.getStorageSync('setUserData');if (value) {//有登陆信息
          console.log("已登录用户：", value);_this.$store.dispatch("setUserData", value); //存入状态
          uni.reLaunch({ url: '../../../pages/index' });}} catch (e) {// error
      }}, startLogin: function startLogin() {//登陆
      if (this.phoneData.length == "") {uni.showToast({ icon: 'none', position: 'bottom', title: '用户名不能为空' });return;}if (this.passData.length < 5) {uni.showToast({ icon: 'none', position: 'bottom', title: '密码不正确' });return;}uni.showLoading({ title: '登陆中' });
...
//# sourceMappingURL=../../../../.sourcemap/mp-weixin/pages/mine/children/login.js.map