			...
			<scroll-view scroll-x="true" class="page_motion_tag">
				...
				<view class="flex">
					<view class="flex-sub" v-for="(item,index) in ydList" :key="index">
						<view class="cu-card article " @tap="ydToUrl({'title':item.title,'bgcolor':item.bgcolor,'title_0':item.title_0,'content_0':item.content_0,'title_1':item.title_1,'content_1':item.content_1})">  <!--跳转到详情-->
							<view class="cu-item bg-img bg-img-yd shadow-warp " :style="[{backgroundImage:'url('+item.img+')'}]" >
								<view class="cu-yd" :style="[{backgroundColor:item.bgcolor}]">
